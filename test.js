	...
	tinymce.init({
		...
		setup: function (editor) {
			...
			editor.on('init', function () {
				// Set HTML hợp đồng vào giao diện
...
function addEventOnclickBPMnew(htmlId, bpm, bpmType) {
	try {
			...
			bpm.context.element.querySelector("input").addEventListener("focus", function () {
				var htmlEle = tinyMceDocument.getElementById(htmlId);
				htmlBeforeFocused.id = htmlId;
				htmlBeforeFocused.backcolor = htmlEle.style.backgroundColor;
				htmlEle.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'center' });
			...
			bpm.context.element.querySelector("input").addEventListener("blur", function () {
				if (tinyMceDocument.getElementById(htmlBeforeFocused.id)) {
					tinyMceDocument.getElementById(htmlBeforeFocused.id).style.backgroundColor = htmlBeforeFocused.backcolor;
				}
			});
		...
		console.error("ERROR -- addEventFocusOnBpmUInew ---" + htmlId + "--- " + e.toString());
	}
}
...
// =================== START update thông tin các trường vào HTML với list ==========================
this.insertHtmlIntoElementById2 = insertHtmlIntoElementById2;
function insertHtmlIntoElementById2(elementId, htmlString) {
    try {
        // Lấy phần tử bằng ID
...
function insertHtmlIntoElementById(elementId, htmlString) {
    try {
        ...
        // Kiểm tra phần tử có tồn tại không
        if (!element) {
            console.warn(`insertHtmlIntoElementById Phần tử với ID '${elementId}' không tồn tại.`);
            return;
        }

        // Thêm HTML vào trước phần tử
        element.insertAdjacentHTML("beforebegin", htmlString);

        console.log(`Đã thêm HTML trước phần tử có ID: ${elementId}`);
    } catch (error) {
        console.error(`Lỗi khi thêm HTML vào trước phần tử có ID '${elementId}':`, error);
    }
}


// hàm kiểm tra tồn tại ID đó chưa ?
this.checkIdInHtml = checkIdInHtml;
function checkIdInHtml(elementId) {
    try {
        // Lấy phần tử bằng ID
		var element = tinyMceDocument.getElementById(elementId);
        ...
        // Kiểm tra phần tử có tồn tại không
        if (!element) {
            console.warn(`checkIdInHtml Phần tử với ID '${elementId}' không tồn tại.`);
            return true;
        }else{
            return false;
        }
...
function removeElementById(elementId) {
    try {
        ...
        // Kiểm tra phần tử có tồn tại không
        if (!element) {
            console.warn(`removeElementById Không tìm thấy phần tử với ID: '${elementId}'`);
            return;
        }
        ...
        console.log(`Đã xóa phần tử với ID: '${elementId}'`);
    } catch (error) {
        console.error(`Lỗi khi xóa phần tử với ID '${elementId}':`, error);
    }
}


// =================== END update thông tin các trường vào HTML với list ==========================
